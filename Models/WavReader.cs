    ...
    public static class WavReader
    {
        public static WavData ReadData(string fileName)
        {
            ...
            using (var reader = new BinaryReader(fs))
            {
                ...
                var numberOfChunks = samplesSize / (formatChunk.SampleRate / 20);
                var chunkedSamples = new double[numberOfChunks][];

                for (int i = 0; i < numberOfChunks; i++)
                {
                    chunkedSamples[i] = new double[formatChunk.SampleRate / 20];
                    for (int j = 0; j < formatChunk.SampleRate / 20; j++)
                    {
                        chunkedSamples[i][j] = samples[formatChunk.SampleRate / 20 * i + j];
                    }
                }
